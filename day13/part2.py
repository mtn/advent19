...
class IntcodeComputer(object):
    def __init__(self, memory, inputs, input_fn=None):
    ...
    def step(self, with_logging):
        ...
        if opcode == "03":  # input
            ...
            if self.inputs_ind == len(self.inputs):
                assert self.inputs is not None
                self.inputs.append(self.input_fn())
...
computer = IntcodeComputer(
    memory, [], input_fn=lambda: (ball_x > paddle_x) - (ball_x < paddle_x)
)
